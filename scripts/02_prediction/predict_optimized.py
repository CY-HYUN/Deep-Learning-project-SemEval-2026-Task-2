# ============================================================================
# Subtask 2a - Test Data Prediction with FINAL Ensemble
# ============================================================================
#
# FINAL OPTIMIZED: seed777 + arousal_specialist (2-model ensemble)
#
# Final Models:
# - seed777: CCC 0.6554 (Best general model)
# - arousal_specialist: CCC 0.6512, Arousal CCC 0.5832 (Arousal specialist)
#
# Requirements:
# - 2 trained models: subtask2a_seed777_best.pt, subtask2a_arousal_specialist_seed1111_best.pt
# - Test data: test_subtask2a.csv with is_forecasting_user marker
# - Ensemble weights from: results/subtask2a/optimal_ensemble.json
#
# Output:
# - pred_subtask2a.csv (user_id, pred_state_change_valence, pred_state_change_arousal)
#
# Expected Performance: CCC 0.6733-0.6933 (Average: 0.6833)
#
# ============================================================================

"""
Subtask 2a - FINAL Test Prediction Script
==========================================
2-Model Ensemble: seed777 + arousal_specialist
Final optimization achieving CCC 0.6833
"""
...
MODEL_PATHS = {
    ...
    'arousal_specialist': f'{MODEL_DIR}/subtask2a_arousal_specialist_seed1111_best.pt'
}

# Load ensemble weights from optimal_ensemble.json
ENSEMBLE_WEIGHTS_PATH = f'{RESULTS_DIR}/optimal_ensemble.json'

print(f'\n✅ Using models: {list(MODEL_PATHS.keys())}')
...
try:
    ...
    print(f'✓ Loaded optimal ensemble weights:')
    for model, weight in weights.items():
        print(f'  {model}: {weight:.4f}')
    print(f'\nExpected CCC: {ensemble_info["ccc_min"]:.4f} - {ensemble_info["ccc_max"]:.4f}')
    print(f'Average CCC: {ensemble_info["ccc_avg"]:.4f}')
except FileNotFoundError:
    print('⚠️ Ensemble weights file not found, using 50:50 weights')
    weights = {'seed777': 0.5, 'arousal_specialist': 0.5}
...
for seed_name, model_path in MODEL_PATHS.items():
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader, desc=f'{seed_name} prediction'):
            ...
            if seed_name == 'seed777':  # Only collect user_ids once
...
print('\n' + '='*80)
print('Next Steps:')
print('1. Run: python scripts/data_analysis/subtask2a/validate_predictions.py')
print('2. Create submission.zip with pred_subtask2a.csv')
print('3. Upload to Codabench: https://www.codabench.org/competitions/9963/')
print('4. Expected CCC: 0.6733-0.6933 (Avg: 0.6833)')
print('='*80)
