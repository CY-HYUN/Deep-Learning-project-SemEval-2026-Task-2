...
Original file is located at
    https://colab.research.google.com/drive/15Qe5ZAhclzwTJOpFJhLwjBvs3yftJLcY
"""

# ============================================================================
# Arousal Specialist Model Training
# ============================================================================
#
# Problem: Arousal CCC (0.55) << Valence CCC (0.76)
# Solution: Train a separate model specialized for Arousal
#
# Key Changes:
# 1. Drastically increase Arousal Loss Weight (70% → 90%)
# 2. Weight samples with high Arousal change
# 3. Add Arousal-specific features
#
# Expected Performance:
# - Arousal CCC: 0.60-0.65 (+9-18% vs current best 0.5516)
# - Final Ensemble: CCC 0.66-0.70
#
# ============================================================================
...
=========================
A model specialized for Arousal prediction
"""
...
print(f'='*80)
print(f'AROUSAL SPECIALIST MODEL - SEED {RANDOM_SEED}')
print(f'='*80)
print(f'Model will be saved as: {MODEL_SAVE_NAME}')
print(f'Goal: Arousal CCC 0.60+ (current best 0.5516)')
print(f'='*80)
...
# Arousal change magnitude (important!)
...
# Arousal acceleration
df['arousal_acceleration'] = df.groupby('user_id')['arousal_change'].diff().fillna(0)

print(f'✓ Arousal specialist features added:')
print(f'  - arousal_change (mean: {df["arousal_change"].mean():.4f})')
print(f'  - arousal_volatility (mean: {df["arousal_volatility"].mean():.4f})')
print(f'  - arousal_acceleration (mean: {df["arousal_acceleration"].mean():.4f})')
...
if USE_WANDB:
    try:
        wandb.init(
            project="semeval-2026-task2-subtask2a-arousal-specialist",
            name=f"arousal-specialist-seed{RANDOM_SEED}",
            settings=wandb.Settings(init_timeout=180),
            config={
                "version": "AROUSAL-SPECIALIST",
                "seed": RANDOM_SEED,
                ...
                "lr_other": 8e-5,
                "weight_decay": 0.01,
                "ccc_weight_valence": 0.50,  # Reduced
                "ccc_weight_arousal": 0.90,  # INCREASED
                "mse_weight_valence": 0.50,
                "mse_weight_arousal": 0.10,  # Reduced
                "arousal_specialist": True
            }
        )
...
print('='*80)
print('AROUSAL SPECIALIST MODEL')
print('='*80)
print(f'Random Seed: {RANDOM_SEED}')
print(f'User Embedding: 64 dim')
print(f'LSTM Hidden: 256')
print(f'Dropout: 0.2')
print(f'Arousal CCC Weight: 90% ⭐⭐⭐ (vs 70% in baseline)')
print('='*80)
...
# ===== AROUSAL SPECIALIST LOSS WEIGHTS =====
CCC_WEIGHT_V = 0.50  # Valence: auxiliary role
CCC_WEIGHT_A = 0.90  # ⭐ Arousal: primary focus (90%)
MSE_WEIGHT_V = 0.50
MSE_WEIGHT_A = 0.10  # ⭐ Reduced MSE weight
...
print(f'Valence Loss: {CCC_WEIGHT_V*100:.0f}% CCC + {MSE_WEIGHT_V*100:.0f}% MSE (auxiliary)')
print(f'Arousal Loss: {CCC_WEIGHT_A*100:.0f}% CCC + {MSE_WEIGHT_A*100:.0f}% MSE ⭐ SPECIALIST')
print('='*80)
...
class EmotionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # ⭐ AROUSAL SPECIALIST: 3 additional features
        temp_features = seq_data[[
            ...
            'arousal_change', 'arousal_volatility', 'arousal_acceleration'  # ⭐ Added
...
# ===== WEIGHTED SAMPLING FOR AROUSAL =====
print('\n=== WEIGHTED SAMPLING ===')
# Higher weights for samples with large Arousal change
...
print(f'  High arousal change (>0.5): {(train_df["arousal_change"] > 0.5).sum()} samples')
...
class FinalEmotionModel(nn.Module):
    def __init__(self, num_users, user_emb_dim=64, lstm_hidden=256, lstm_layers=2,
        ...
        temp_feature_dim = 20  # ⭐ 17 → 20 (3 arousal features added)
        user_stat_dim = 4
        text_feature_dim = 10
...
def dual_head_loss(valence_pred, arousal_pred, valence_true, arousal_true,
    ...
    """Arousal Specialist Loss"""
    # Valence loss (auxiliary)
    ...
    # Arousal loss (specialist - 90% CCC)
...
print('\n=== TRAINING AROUSAL SPECIALIST ===\n')

best_ccc = -1
best_arousal_ccc = -1  # Track best Arousal CCC separately
patience_counter = 0
...
for epoch in range(NUM_EPOCHS):
    ...
    if val_ccc > best_ccc:
        ...
        torch.save({
            ...
            'best_ccc': best_ccc,
            'best_arousal_ccc': best_arousal_ccc,
            'val_ccc_v': val_ccc_v,
            'val_ccc_a': val_ccc_a,
        ...
        print(f'  ✓ Best model saved! (CCC: {best_ccc:.4f}, Arousal CCC: {best_arousal_ccc:.4f})')

        # Also save to Google Drive (backup)
        try:
            import shutil
            os.makedirs('/content/drive/MyDrive/models', exist_ok=True)
            drive_save_path = f'/content/drive/MyDrive/models/{MODEL_SAVE_NAME}'
            shutil.copy(MODEL_SAVE_NAME, drive_save_path)
        ...
        if USE_WANDB:
            ...
            wandb.run.summary['best_arousal_ccc'] = best_arousal_ccc
...
print(f'Best Arousal CCC: {best_arousal_ccc:.4f} ⭐ (Goal: 0.60+)')
...
print('\n' + '='*80)
print(f'AROUSAL SPECIALIST TRAINING COMPLETE!')
print(f'Expected Arousal CCC: 0.60-0.65')
print(f'Expected Final Ensemble CCC: 0.66-0.70 (with baseline models)')
print('='*80)
print('\nNext steps:')
print(f'1. Use this model in ensemble for Arousal predictions')
print(f'2. Use baseline models (seed123, seed777, seed888) for Valence predictions')
print(f'3. Combine predictions with optimal weights')
print('='*80)