...
**Evaluation Metrics**:
- **Pearson r**: Correlation between predicted and gold state change values per user
- **MAE**: Mean Absolute Error per user
- **Leaderboard Ranking**: Based on Pearson r
...
## 📊 Evaluation Metrics (Released Dec 6, 2025)
...
**Official Evaluation Script**: Available on [GitHub](https://github.com/semeval2026task2)

---

### Subtask 1: Longitudinal Affect Assessment - Evaluation

For each outcome (valence and arousal), evaluation is based on **three correlation measures**:

#### 1. Between-User Correlation

For each user, the mean predicted text score and mean gold text score are computed across all texts. Pearson r is then calculated across users:

```
r_between = r({mean_per_user(ŷ)}, {mean_per_user(y)})
```

Similarly, MAE_between is computed using the same per-user means.

**What it measures**: How well the model captures differences between users (e.g., Person A is generally happier than Person B)

---

#### 2. Within-User Correlation

For each user, Pearson r is computed between predicted and gold text scores across that user's texts. These per-user correlations are then averaged across users:

```
r_within = mean_over_all_users(r_per_user(ŷ, y))
```

Similarly, MAE_within is computed per user and then averaged.

**What it measures**: How well the model captures variation within each user (e.g., Text 1 made Person A happier than Text 2)
...
Similarly for MAE_composite.

**Leaderboard Ranking**: Based on **r_composite** (composite correlation)

**Additional Display**: Between-user and within-user results will be highlighted separately

---

### Subtask 2A: State Change Forecasting - Evaluation ⭐

**Metrics Calculated**:

1. **Pearson r** between predicted and gold state change values per user:
   ```
   r(ŷ_user, y_user) for valence and arousal separately
   ```

2. **MAE** between predicted and gold state change values per user:
   ```
   MAE(ŷ_user, y_user) for valence and arousal separately
   ```

**Leaderboard Ranking**: Based on **Pearson r**

**What it measures**: How accurately the model predicts the direction and magnitude of emotional change for each individual user

---

### Subtask 2B: Dispositional Change Forecasting - Evaluation

**Metrics Calculated**:

1. **Pearson r** between predicted and gold disposition change values per user:
   ```
   r(ŷ_user, y_user) for valence and arousal separately
   ```

2. **MAE** between predicted and gold disposition change values per user:
   ```
   MAE(ŷ_user, y_user) for valence and arousal separately
   ```
...
**What it measures**: How accurately the model predicts long-term trends in emotional disposition for each individual user

---

### Test Data Release Information

#### Subtask 1
- **Evaluation Data**: 1,737 longitudinal texts from 91 authors
- **Additional Marker**: `is_seen_user` (boolean) - identifies users whose partial data was in training set

#### Subtask 2 (2A and 2B)
- **Evaluation Data**: No additional data release
- **Additional Marker**: `is_forecasting_user` (boolean) - identifies users whose partial data was in training set and will be evaluated

**Expected Release**: Mid-December 2025 (TBA)
